	...
	* buildings in other words are NOT made up of components - much quicker! */
	...
	if (!defensive)
	{
		...
		if (psStructure->pStructureType->pBaseIMD != NULL)
		{
			...
			pie_Draw3DShape(psStructure->pStructureType->pBaseIMD, 0, colour, buildingBrightness, pieFlag, pieFlagData, 128);
		}
	...
	{
		pie_Draw3DShape(strImd, 0, colour, buildingBrightness, pie_HEIGHT_SCALED | pie_SHADOW, structHeightScale(psStructure) * pie_RAISE_SCALE, 128);
	}
	else
	{
		if (structureIsBlueprint(psStructure))
		...
		pie_Draw3DShape(strImd, animFrame, colour, buildingBrightness, pieFlag, pieFlagData, 128);
		...
		*/
			...
			(psStructure->status == SS_BEING_BUILT && psStructure->pStructureType->type == REF_RESOURCE_EXTRACTOR) )
		{
			pie_Draw3DShape(psStructure->sDisplay.imd, 0, getPlayerColour(psStructure->player),
			                brightness, pie_HEIGHT_SCALED|pie_SHADOW, structHeightScale(psStructure) * pie_RAISE_SCALE, 128);
		}
		else
		{
			if (structureIsBlueprint(psStructure))
			...
			pie_Draw3DShape(imd, 0, getPlayerColour(psStructure->player), brightness, pieFlag, pieFlagData, 128);
		}
		imd->points = temp;
