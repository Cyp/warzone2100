...
template <int N>
static uint32_t randHash(uint32_t const (&data)[N]) {
	...
	for (int i = 0; i < N; ++i) {
		shuffle(data[i], 0x7ea99999);
	}
	for (int i = 0; i < N; ++i) {
		shuffle(data[i], 0xc0ffee77);
	}
	return v;
}
...
static void addConstructionLine(DROID *psDroid, STRUCTURE *psStructure, const glm::mat4 &viewMatrix)
{
	...
	int amount = 400 * constructPoints * (graphicsTime - psDroid->actionStarted) / GAME_TICKS_PER_SEC;

	Vector3i each;
	auto getPoint = [&](uint32_t c) {
		uint32_t t = (amount + c)/1000;
		float s = (amount + c)%1000*.001f;
		unsigned pointIndexA = randHash((uint32_t[]){psDroid->id, psStructure->id, psDroid->actionStarted, t, c}) % psStructure->sDisplay.imd->points.size();
		unsigned pointIndexB = randHash((uint32_t[]){psDroid->id, psStructure->id, psDroid->actionStarted, t + 1, c}) % psStructure->sDisplay.imd->points.size();
